...
class TaobaoDeliveryTemplateAddRequest(BaseRequest):
...
<br/><br/><font color=red>注意：填入该值时必须取您的发货地址最小区域级别ID，比如您的发货地址是：某省XX市xx区（县）时需要填入区(县)的ID，当然有些地方没有区或县可以直接填市级别的ID</font>
        """
        self._consign_area_id = consign_area_id
        """
...
<br/><br/><font color=red>输入的格式分号个数和template_types数量一致，逗号个数必须与template_dests以分号隔开之后一一对应的数量一致</font>
    ...
    def valuation(self, valuation):
        if isinstance(valuation, int):
            ...
            raise TypeError("valuation must be int")

    @property
    def consign_area_id(self):
        return self._consign_area_id

    @consign_area_id.setter
    def consign_area_id(self, consign_area_id):
        if isinstance(consign_area_id, int):
            self._consign_area_id = consign_area_id
        else:
            raise TypeError("consign_area_id must be int")
    ...
    def to_dict(self):
        ...
        if self._valuation is not None:
            request_dict["valuation"] = convert_basic(self._valuation)

        if self._consign_area_id is not None:
            request_dict["consign_area_id"] = convert_basic(self._consign_area_id)
